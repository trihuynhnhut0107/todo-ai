        ...
        for (const change of changes) {
          switch (change.type) {
            case "MODIFIED_ON_DEVICE":
              ...
              console.log(
                `[API CALL] Update Event ${change.appEventId} theo device data:`,
                change.diff
              );
              break;
            ...
            case "DELETED_ON_DEVICE":
              ...
              console.log(
                `[API CALL] Xóa Event ${change.appEventId} vì đã mất trên device.`
              );
              break;
            ...
            case "CREATED_ON_DEVICE":
              ...
              console.log(
                `[API CALL] Import Event mới từ device:`,
                change.diff?.title
              );
              break;
          }
        ...
        if (data.silent) {
          ...
          try {
            if (type === "event_created" && event) {
              ...
              const calendarEventId = await addEventToCalendar(payload);

              // Send calendarEventId back to backend
              if (calendarEventId && event.workspaceId) {
                console.log(
                  "Sending calendarEventId to backend:",
                  calendarEventId
                );
                try {
                  await updateEvent({
                  ...
                  console.log(
                    "✅ Successfully saved calendarEventId to backend"
                  );
                } catch (error) {
                  console.error(
                    "❌ Failed to save calendarEventId to backend:",
                    error
                  );
                }
              }
            ...
            } else if (type === "event_deleted" && data.calendarEventId) {
              console.log("Xử lý xóa sự kiện lịch:", data.calendarEventId);
              await removeTodoFromCalendar(data.calendarEventId);
            }
          } catch (error) {
...
async function registerForPushNotificationsAsync() {
  ...
  if (!projectId) {
    console.warn(
      "⚠️ Không tìm thấy projectId trong app.json (expo.extra.eas.projectId)."
    );
    return;
  }
...
export async function checkCalendarDrift(
  ...
  try {
    ...
    for (const appEvent of appEvents) {
      ...
      const native = nativeEvents.find(
        (n) => n.id === appEvent.calendarEventId
      );
      ...
      if (!native) {
        ...
        if (
          (appEvent.description || "").trim() !== (native.notes || "").trim()
        ) {
        ...
        // 3. So sánh Location
        if (
          (appEvent.location || "").trim() !== (native.location || "").trim()
        ) {
        ...
        const appStart = new Date(
          appEvent.deadline || appEvent.startDate
        ).getTime();
